...
is higher, fig. \ref{fig:dTrueFit} right, because misfit in $d$ with small values
has a minor effect on the mispredictions of the read number due to 
an exponential dependance (e.g. $\mu e^{-dt}$), fig. \ref{fig:errInD}.
Hence, genes with low degradation rate are not 
```{r include=FALSE}
cap <- paste0(
 "Contribution of the error in the degradation rate $d$ estimation to ",
 "the mean read number predition $\\mu e^{-dt}$ for the time point $t=1$. ",
...
pulse-chase RNA-seq experiments goes beyound the skope if this application note.
...
issues if the system is ill-posed and responsible and thorough 
