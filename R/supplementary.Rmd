...
parmeter estimations with their true values used in simulation.
...
\text{[labelled fraction]} &= \alpha_1 \text{[labelled RNA]} +
\alpha_2 \text{[unlabelled RNA]}\\
\text{[unlabelled fraction]} &= \alpha_3 \text{[unlabelled RNA]} +
...
One can simulate count data using internal pulseR function
`pulseR:::generateTestDataFrom`. This function takes as an input
formulas for the mean read counts, a condition matrix 
(i.e. condition $\times$ time), normalisation factors and the parameter values
